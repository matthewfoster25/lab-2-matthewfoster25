...
```{r}
#| label: num-observations-nobel
#| eval: true

num_observations <- nrow(nobel)
...
```{r}
#| label: num-variables-nobel
#| eval: true

num_variables <- ncol(nobel)
...
num_observations_nobel_living <- nrow(nobel_living)
num_observations_nobel_living
```
...
```{r}
#| label: laureate-US-relationship
#| eval: true
...
ggplot(nobel_living_science, aes(x = country_us, fill = country_us)) +
    ...
    coord_flip() +
    labs(title = "Relationship Between Prize Category and Laureate Winner Location",
         x = "Laureate Winner Location",
         y = "Count")
```

**Interpretation:**
The bar graph shows that the majority of Laureate Winners were in the USA for all 4 categories of Chemistry, Economics, Medicine, and Physics. However, the only bar with a resounding majority is economics. This supports the headline that immigration is important for American Science as more Chemists, Phsyisicts, and Medicinal Workers need to immigrate to America in order to have similar graphs to Economics.
...
count_us_born_winners <- sum(nobel_living_science$born_country_us == "USA")

count_us_born_winners
...
```{r}
#| label: laureate-US-relationship-2
#| eval: true
...
**Interpretation:**
The secondary graph, showing the immigrated Nobel winners in the US, shows that there are more winners born elsewhere then in the USA. For this reason, Buzzfeed's claim is true, immigration is important for American science.
